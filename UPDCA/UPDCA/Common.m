...
+ (BOOL)createPath:(NSString *)path
{
    ...
    if(![dir createDirectoryAtPath:path
...
+ (BOOL)createFile:(NSString *)pathAndFileName
           content:(NSString *)content
{
    NSFileManager * fm = [NSFileManager defaultManager];
    if(![fm createFileAtPath:pathAndFileName
                    contents:[content dataUsingEncoding:NSASCIIStringEncoding]
                  attributes:nil])
    {
        return FALSE;
    }
...
+ (NSString *)readFileDataToBuffer:(NSString *)path
{
    ...
    buffer = [NSString stringWithContentsOfFile:path
